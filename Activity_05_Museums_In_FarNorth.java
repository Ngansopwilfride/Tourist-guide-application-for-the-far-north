...
public class Activity_05_Museums_In_FarNorth extends AppCompatActivity implements View.OnClickListener {

    Button btn0505; //Logout
    String uName;
    TextView userName;
    Intent intent01, intentView;
    int n; // number of an museums
    int k = 0;
    DataSnapshot dataSnapshot;
    Attractions attraction;
    final List<Attractions> listAtr = new ArrayList<>(); // Attractions objects arrayList

    private static final String TAG = "Activity_05_Museums";       // for testing in Logcat
    ...
    private ArrayList<String> arrKey = new ArrayList<>();          // arrayList with database keys (museums)
    private ArrayList<String> mLocation = new ArrayList<>();       // arrayList with museums locations
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_05_museums_in_farnorth);

        toAttraction07=new Intent(this,Activity_07_Current_Attraction.class);

        btn0505 = (Button) findViewById(R.id.button0505); btn0505.setOnClickListener(this); // ---> Logout

        userName = (TextView) findViewById(R.id.textView0501);
        Intent getMemory = getIntent();
        uName = getMemory.getStringExtra("uName2");
        userName.setText(uName); uName3=uName;
        category = getMemory.getIntExtra("category",5);  // the category is the attraction type: 5 - Museums
        ...
        ref = FirebaseDatabase.getInstance().getReference("museums");  // ref (from the DatabaseReference type) connection with the "museums" database
        ...
        ValueEventListener listener = new ValueEventListener() { // transferring the database to its corresponding arrayLists
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot dss: dataSnapshot.getChildren()) {
                    ...
                    arrKey.add(dataSnapshot.getKey());  //arrayListr <key> - for Museums
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.button0505) logout01();
    }
